...
@register
class ccAdaDelta(Optimizer):
    """The ccAdaDelta optimizer.
    ...
    def __init__(self, rho=0.95, epsilon=1e-6, **kwargs):
        super(ccAdaDelta, self).__init__(**kwargs)
        self.rho = rho
        self.epsilon = epsilon
    ...
    def create_state(self, index, weight):
        return (zeros(weight.shape, weight.context, dtype=weight.dtype), #accumulated g
                zeros(weight.shape, weight.context, dtype=weight.dtype)) #accumulated delta
    ...
    def update(self, index, weight, grad, state):
        ...
        lr = self._get_lr(index)
        wd = self._get_wd(index)
        self._update_count(index)#reordered, fixed by hjchen2
...
#added by yflv for parallel bmuf algorithm
@register
class BMUF(Optimizer):
    """Optimizer for merge weights on master node.

    Parameters
    ----------
    blr : float, optional
        learning_rate of bmuf

    bm: float, optional
       momentum value of bmuf

    """
    def __init__(self, blr=1.0, bm=0.75, **kwargs):
        super(BMUF, self).__init__(**kwargs)
        self.blr = blr
        self.bm  = bm
        self.kwargs ={'blr': blr,'bm':bm,
                      'rescale_grad':self.rescale_grad}
    ...
    def update(self, index, weight, grad, state):
        ...
        wd = self._get_wd(index)
        # Nestrove updater
        mom, weight_server = state
        
        bmuf_update(weight,grad,mom,weight_server,
                        out=weight,
                        lr=lr,wd=wd,**self.kwargs) 
