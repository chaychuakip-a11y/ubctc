...
def _norm_clipping(grad_arrays, num_device, threshold):
    assert isinstance(grad_arrays, list)
    for i in range(num_device):
        norm_val = np.sqrt(sum([nd.norm(grad[i]).asnumpy()[0]**2 
            for grad in grad_arrays]))
        ratio = 1.0
        #print "xxxx: ", norm_val
        if norm_val > threshold:
            logging.info("clip value: %f", norm_val)
...
# added by yflv for bmuf algorithm
def _update_params_bmuf(param_arrays, grad_arrays, updater, num_device, nbatch, kvstore, sync_freq, alpha):
    """ Perform update of param_arrays from param_arrays with bmuf."""
    # TODO
    for index, pair in enumerate(zip(param_arrays, grad_arrays)):
        arg_list, grad_list = pair
        if grad_list[0] is None:
        ...
        # update models of each device using local gradient
        for k, p in enumerate(zip(arg_list, grad_list)):
            # faked an index here, to make optmizer create diff
            # state for the same index but on diff devs, TODO(mli)
            # use a better solution latter
            w, g = p
            updater(index*num_device+k, g, w)

        if (nbatch + 0) % sync_freq != 0:
            continue
        ...
        # perform easgd operation: w[t+1] = (1-alpha)*w[t] + alpha*wcenter[t+1]
        for k, p in enumerate(zip(arg_list, grad_list)):
            w, g = p
            w[:] = (1.0 - alpha) * w + alpha * g
...
def _train_multi_device(symbol, ctx, arg_names, param_names, aux_names,
                        ...
                        begin_epoch, end_epoch, epoch_size, eval_size,
                        ...
                        #added by yflv for bmuf
                        use_bmuf=False, sync_freq=1, alpha=1, blr=1, bm=0,
                        mutable_data_shape=False, max_data_shape=None, clip_gradient=-1.0):
    ...
    # modified by yflv for compatible with bmuf
    if update_on_kvstore:
        if use_bmuf == False:
            kvstore.set_optimizer(optimizer)
        else:
            total_devices = kvstore.num_workers * len(ctx)
            master_optimizer = opt.create('bmuf',
                                   rescale_grad=(1.0/total_devices),
                                   blr=blr, bm=bm)
            kvstore.set_optimizer(master_optimizer)
            updater = get_updater(optimizer)
    ...
    for epoch in range(begin_epoch, end_epoch):
        ...
        while True:
            ...
            for data_batch in train_data:
                ...
                #modified by yflv for bmuf
                if clip_gradient > 0:
                    _norm_clipping(executor_manager.grad_arrays, len(ctx), clip_gradient)
                ...
                if update_on_kvstore:
                    if use_bmuf == False:
                        ...
                        # TODO
                        _update_params_bmuf(executor_manager.param_arrays,
                                executor_manager.grad_arrays,
                                updater=updater,
                                num_device=len(ctx),
                                nbatch=nbatch,
        ...
        if eval_data:
            ...
            for i, eval_batch in enumerate(eval_data):
                ...
                if eval_size is not None and total_num_batch >= eval_size:
                    break
...
class FeedForward(BASE_ESTIMATOR):
    ...
    def __init__(self, symbol, ctx=None,
                 num_epoch=None, epoch_size=None, eval_size=None, optimizer='sgd',
        ...
        # added by yflv for fully-cnn
        self.mutable_data_shape  = self.kwargs.pop('mutable_data',  False)

        # added by yflv for sequence trainging
        self.clip_gradient  = self.kwargs.pop('clip_gradient',  -1.0)

        # added by yflv for bmuf
        self.use_bmuf  = self.kwargs.pop('use_bmuf',  False)
        self.sync_freq = self.kwargs.pop('sync_freq', 1)
        self.alpha     = self.kwargs.pop('alpha', 1.0)
        self.blr       = self.kwargs.pop('blr',   1.0)
        self.bm        = self.kwargs.pop('bm',    1.0)
    ...
    def fit(self, X, y=None, eval_data=None, eval_metric='acc',
        ...
        _train_multi_device(self.symbol, self.ctx, arg_names, param_names, aux_names,
                            ...
                            use_bmuf=self.use_bmuf, sync_freq=self.sync_freq, 
                            alpha=self.alpha, blr=self.blr, bm=self.bm)
