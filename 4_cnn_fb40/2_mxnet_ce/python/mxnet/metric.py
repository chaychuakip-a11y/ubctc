...
class Accuracy(EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        preds : list of `NDArray`
            Predicted values.
        """
        new_shape = (preds[0].shape[0], labels[0].size / preds[0].shape[0])
        labels[0] = labels[0].reshape(new_shape)
        ...
        for i in range(len(labels)):
            ...
            check_label_shapes(label, pred_label)
            #added by yanli6 for lstm and ignore label
            if label.shape[1] == 1:
                #self.sum_metric += (pred_label.flat == label.flat).sum()
                for pred_block, label_block in zip(pred_label, label):
                    if label_block != -1:
                        self.num_inst += 1
                        if label_block == pred_block:
                            self.sum_metric += 1
            else:
                mask = label[:,1]
                label = label[:,0]
                for pred_block, label_block, mask_block in zip(pred_label, label, mask):
                    if mask_block < 2:
                        self.num_inst += 1
                        if label_block == pred_block:
                            self.sum_metric += 1
...
@register
@alias('lm_ce')
class LMCrossEntropy(EvalMetric):
    def __init__(self, name='lm-cross-entropy',
            output_names=None, label_names=None):
        super(LMCrossEntropy, self).__init__(
            name, output_names=output_names,
    ...
    def update(self, labels, preds):
        ...
        for label, pred in zip(labels, preds):
            ...
            (len_batch_size, vocab_size) = pred.shape 

            label_mask     = labels[1]
            label          = label.reshape((batch_size*seq_len,))
            label_mask     = label_mask.reshape((batch_size*seq_len,))
            label          = label*label_mask
            prob           = ndarray.choose_element_0index(pred, label.as_in_context(pred.context))
            log_prob       = (-numpy.log(prob.asnumpy() + 1e-10))*label_mask.asnumpy()
      
            self.sum_metric += (log_prob.sum())
            self.num_inst   += label_mask.asnumpy().sum()

@register
@alias('msemask')
class MSE_mask(EvalMetric):
    """Calculate Mean Squared Error loss"""
    def __init__(self, name='mse_mask',
            output_names=None, label_names=None):
        super(MSE_mask, self).__init__(
            name, output_names=output_names,
            label_names=label_names
            )
    ...
    def update(self, labels, preds):
        for label, pred in zip(labels, preds):
            pred = pred.asnumpy()
            label = label.asnumpy()[:,:pred.shape[1]]
...
@register
@alias('lm_nce')
class NCE(EvalMetric):
    def __init__(self):
        super(NCE, self).__init__('nce')
...
@register
@alias('lm_ppl')
class PPL(EvalMetric):
    def __init__(self):
        super(PPL, self).__init__('lm_ppl')
    ...
    def get(self):
        ...
        if self.num_inst == 0:
            ...
            return (self.name, numpy.exp(self.sum_metric / self.num_inst))
