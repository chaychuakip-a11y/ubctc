...
class Module(BaseModule):
    ...
    def __init__(self, symbol, data_names=('data',), label_names=('softmax_label',),
        ...
        self._nbatch_processed = 0 # for bmuf
    ...
    def init_optimizer(self, kvstore='local', optimizer='sgd',
        ...
        if kvstore and 'dist' in kvstore.type and '_sync' in kvstore.type:
            rescale_grad /= (kvstore.num_workers * len(self._context))


        # args for bmuf
        self._use_bmuf  = optimizer_params.pop('use_bmuf', False)
        self._sync_freq = optimizer_params.pop('sync_freq', 1)
        self._alpha     = optimizer_params.pop('alpha', 1.0)
        self._blr       = optimizer_params.pop('blr', 1.0)
        self._bm        = optimizer_params.pop('bm', 1.0)
        self._clip_norm = optimizer_params.pop('clip_norm', -1.0)
        ...
        if update_on_kvstore:
            if not self._use_bmuf:
                kvstore.set_optimizer(self._optimizer)
            else:
                total_devices= kvstore.num_workers*len(self._context)
    ...
    def borrow_optimizer(self, shared_module):
        ...
        self._use_bmuf        = shared_module._use_bmuf
        self._sync_freq       = shared_module._sync_freq
        self._alpha           = shared_module._alpha
        self._blr             = shared_module._blr
        self._bm              = shared_module._bm
        self._clip_norm       = shared_module._clip_norm
    ...
    def _norm_clipping(self, grad_arrays):
        threshold = self._clip_norm
        for i in range(len(self._context)):
            norm_val = numpy.sqrt(sum([nd.norm(grad[i]).asnumpy()[0]**2 for grad in grad_arrays]))
            ratio = 1.0
    ...
    def update(self):
        ...
        if self._clip_norm > 0.0:
            self._norm_clipping(self._exec_group.grad_arrays)
        if self._update_on_kvstore:
            if self._use_bmuf:
                #self.logger.info('sync_freq ={}. nbatch={}, ndev={}'.format(self._sync_freq, self._nbatch_processed, len(self._context)))
                _update_params_bmuf(self._exec_group.param_arrays,
                                    self._exec_group.grad_arrays,
                                    updater=self._updater,
                                    num_device=len(self._context),
                                    nbatch=self._nbatch_processed,
                                    kvstore=self._kvstore,
                                    sync_freq=self._sync_freq,
                                    alpha=self._alpha
                                    )
                self._nbatch_processed += 1
            else:
