...
def get_symbol(seq_len=1024,fea_dim=40, num_classes=9004, batch_num=1, module_num=10, coff=1):
    ...
    labellf = mx.sym.Variable(name="labellf")
    ...
    convlow = DeconvFactory(data=conv,  label=label, num_filter=512,  kernel=(1,1), stride=(1,1))
    ...
    labellf   = mx.sym.Reshape(data=labellf, shape=(-1,))
    labellf   = (labellf < -1) + labellf
    softmax = mx.sym.SoftmaxOutput(data=convout, label=labellf, use_ignore=True, normalization='valid')
    return softmax
