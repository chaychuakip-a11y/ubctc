...
class FullyChunkIter(mx.io.DataIter):
    def __init__(self, path_data, data_shape, batch_size, sampler_type,chunk_size,
        ...
        self.provide_data   = [("data", (batch_size, 1, 40, frame_num)),
                ("mask", (batch_size, frame_num/4, frame_num/4)),("label",(batch_size, frame_num))]
        self.provide_label  = [("labellf",(batch_size, frame_num/4))]
    ...
    def next(self):
        ...
        #####labellf
        label_shape = (1, data_batch.label[0].shape[1])
        labellf_shape = (1, data_batch.label[0].shape[1]/4)
        labellf_data  = np.zeros(labellf_shape)

        #TODO: set labellf
        for i in range(labellf_shape[1]):
            labellf_data[0][i] = data_batch.label[0].asnumpy()[0][i*4]

        #data_batch.data.append(mx.nd.array(labellf_data))
        data_batch.data.append(data_batch.label.pop())
        data_batch.label.append(mx.nd.array(labellf_data))


        data_batch.provide_data  = [("data", data_batch.data[0].shape), ("mask", mask_shape), ("label", label_shape)]
        data_batch.provide_label = [("labellf", labellf_shape)]

        #print "provide_data: ", data_batch.provide_data
        #print "provide_label: ", data_batch.provide_label
