...
my $hdir         = "/workdir/asrdictt/tasrdictt/taoyu/mlg/korean/17kh_wav_aug1.8.wav_fb40_dnnfa";
my $nSplit       = 10;
my $nPart        = 100;
my $states_list  = "/work1/asrdictt/taoyu/mlg/korean/am/1_mle_mfc/final_s9k_for_ctc/cdphone.map.list.nosp";  #### cdphone cluster: cdphone.map.list.nosp; state cluster: states.list
my $scp_filter   = "";
my $descript_fea = "fb40";
my $descript_lab = "mlf_fa_ph";
my $IsCTClab     = "0";  #### "0": cdphone ce label； "1": cdphone ctc label; "": state label;
$IsCTClab eq "0" || $IsCTClab eq "1" || $IsCTClab eq "" || die "Error: informal parameter for IsCTClab: $IsCTClab\n";
...
foreach my $split_id(@split_id)
{
	...
	if(!-e "$pfile_fea.done.finish")
	{
		...
		# $cmd = "/home3/hadoop/hadoop2/bin/hdfs dfs $dfs_param -cat $hdir_src | $bin_pak_low_frame_rate 4 $descript_fea | $bin_qnfiletransfer - $descript_fea $pfile_fea $descript_lab $pfile_lab $states_list $scp_lab $IsCTClab $scp_filter >$trans_log 2>&1";
		# $cmd = "/home3/hadoop/hadoop2/bin/hdfs dfs $dfs_param -cat $hdir_src | $bin_pak_low_frame_rate_labelexpand 4 $descript_fea $descript_lab | $bin_qnfiletransfer - $descript_fea $pfile_fea $descript_lab $pfile_lab $states_list $scp_lab $IsCTClab $scp_filter >$trans_log 2>&1";
		print $cmd."\n";
		system($cmd);
