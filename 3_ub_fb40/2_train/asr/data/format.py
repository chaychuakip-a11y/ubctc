...
@jit.ignore
def clip_mask(x: torch.Tensor, nmod: int, dim: int):
    clip_length = x.shape[dim] // nmod
    if x.shape[dim] % nmod > 0 : clip_length += 1
    indices = torch.arange(clip_length, device=x.device, dtype=torch.long)
    indices = indices * nmod
    cliped_mask = torch.index_select(x, dim, indices).contiguous()
    return cliped_mask
