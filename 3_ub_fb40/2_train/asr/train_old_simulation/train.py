...
def validation(model, config, data_loader, LOG):
    ...
    for i, data_element in enumerate(data_loader):
        ...
        #print("test",i,data.shape,meta.keys())
        criterion = trainer.forward(i, data_element, model) # xiaobao
        # pdb.set_trace()
        if i % config["TrainSetting"].getint("Display") == 0:
            LOG.log("validated {0} / {1}".format(i,len(data_loader)))
        if isinstance(criterion, dict):
            for key in criterion:
                if key not in criterion_holder:
                    criterion_holder[key] = [criterion[key]]
                else:
                    criterion_holder[key].append(criterion[key])
        else:
            key = "loss"
            if key not in criterion_holder:
                criterion_holder[key] = [float(criterion)]
            else:
                criterion_holder[key].append(float(criterion))

    for key in criterion_holder:
        criterion_holder[key] = np.array(criterion_holder[key])
    LOG.log("modelname.. {0}".format(config["TrainSetting"]["CurrentModel"]))

    for key in criterion_holder:
        LOG.log("{0}: {1}".format(key, np.mean(criterion_holder[key])))
