...
my @hdir_src; if (@ARGV > 0) {
    ...
    @hdir_src = (
                   "$config_data->{hdfs_out_root}/17kh_wav_dnnfa/*-part-*[0-1]8",
                   );
}
my $hdir_out     = "$config_data->{hdfs_out_root}/17kh_wav_addnoise_music_a_0.02.wav_dnnfa";
...
my $noisedata    = "$config_data->{hdfs_src_root}/../noisedata/16k/2.700h_pure_music_data.pak";#IN
