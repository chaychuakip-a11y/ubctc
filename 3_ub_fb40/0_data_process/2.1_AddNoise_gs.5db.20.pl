...
my @hdir_src; if (@ARGV > 0) {
    ...
    @hdir_src = (
                   "$config_data->{hdfs_out_root}/17kh_wav_dnnfa/*-part-*[4-5]",
                   );
}
my $hdir_out     = "$config_data->{hdfs_out_root}/17kh_wav_addnoise_gs_0.2.wav_dnnfa";
...
my $noisedata    = "$config_data->{hdfs_src_root}/../noisedata/16k/NoiseGS.addKTV.pcm.index.scp.tmp.pak.1";#IN
